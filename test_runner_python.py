...
This variant runs the CLI via the current Python interpreter instead of `uv`.
...
def build_python_cli_command() -> List[str]:
    """Builds the base command to run the analyzer via Python.

    Using sys.executable ensures the same interpreter running this script
    executes the analyzer module, which avoids environment mismatches.
    """
    return [sys.executable, "-m", "static_traffic_analyzer.cli"]
...
def run_case(case_path: Path, db_is_running: bool) -> bool:
    ...
    if fortigate_conf.is_file():
        ...
        try:
            run_command(
                build_python_cli_command()
                + ["--config", str(fortigate_conf)]
                + common_args
            )
    ...
    if mariadb_sql.is_file():
        if db_is_running:
            ...
            try:
                ...
                run_command(build_python_cli_command() + db_args + common_args)
                if analyze_differences(expected_dir / "expected.csv", OUT_FILE):
                    had_failure = True
...
def main() -> None:
    ...
    print("Starting static-traffic-analyzer test run (python CLI)...")
